...
- 곁다리로 고친 pty 코어 버그: **`ttyname_r` → `ptsname_r`**. M5 에서 "재현
  ...
  부족")로 보고한다. 그래서 버퍼를 의심하게 만든다. 측정: **4000회 중 3648회
  실패**, 버퍼 32B~4KiB 전부 동일. `ptsname_r` 은 같은 pty 에서 4000/4000
  성공하고 이름도 일치했다 — 커널에 직접 묻기 때문(TIOCPTYGNAME). 이 한 줄을
  앞뒤로 바꿔가며 테스트가 **5/5 실패 / 5/5 통과**로 갈렸다
- **정정 (그 뒤에 밝혀진 것)**: 위 측정은 사실이지만, 당시 이 문서와 코드
  주석이 원인으로 지목한 "동시 pty 생성"은 **틀렸다**. 이후 재현되지 않는다 —
  동시성, 생성/파괴 churn, 살아있는 pty 450개, 버퍼 크기, 머신 부하를 각각
  분리해 시험했고 원래 프로브도 지금은 4000/4000 깨끗하다. 무엇이 그 상태를
  만들었는지는 **모른다**. 수정 자체는 유효하다 — 둘 중 하나에는 실패 경로가
  있고 다른 하나에는 없다 — 다만 "조건을 안다"고 적었던 것이 잘못이었다
...
- **pty 슬레이브 이름은 `ptsname_r` 로 얻는다**: macOS 의 `ttyname_r` 은
  /dev 조회로 이름을 찾고 그 실패를 전부 ERANGE 로 보고한다. 한 번 대량으로
  터지는 것을 관측했고(4000회 중 3648회), `ptsname_r` 은 같은 pty 에서
  전부 성공했다. **그 상태를 만든 조건은 아직 모른다** — 이후 재현 실패.
  `ptsname_r` 은 커널에 직접 물어 그 조회 자체가 없다는 점만으로 선택.
...
- 시스템의 pty 총량에 상한이 있다 (macOS `kern.tty.ptmx_max`, 기본 511).
  ...
  말해준다.
- **슬레이브 노드는 이미 `/dev` 아래에 있다** — macOS `/dev/ttysNNN`,
  Linux `/dev/pts/N`. `--link` 심볼릭 링크는 그 위에 얹는 *안정된 이름*일
  뿐이고, 커널이 매 실행 번호를 다시 매기기 때문에 존재한다. 도구는 둘 중
  아무거나 열어도 된다.
- **그 이름을 `/dev` 아래 둘 수 있는지는 OS 마다 다르다** (실측):
  - Linux 는 root 로 가능하다 — devtmpfs 는 평범한 파일시스템이고, udev 가
    `/dev/serial/by-id/*` 를 만드는 것과 같은 일이다. 러너에서 end-to-end
    확인: `/dev/ttyforge0`/`1` 발행, 바이트 왕복, 종료 시 링크 제거.
    **단 포트가 root 소유 `crw--w----` 가 되어 사용자 도구는 EACCES 를
    받는다** — "sudo 로 하면 됩니다"까지만 쓰면 독자에게는 실패하는
    지시문이 된다.
  - macOS 는 불가능하다. devfs 는 **root 에게도** 거부한다: `ln -s` 와
    `touch` 둘 다 EACCES 가 아니라 **EPERM** 을 돌려준다(연산 미지원의
    신호). `/dev` 안의 심볼릭 링크는 커널이 부팅 때 만든 stdin/stdout/stderr
    셋뿐이다.
- **진짜 `cu.*`/`tty.*` 쌍은 만들 수 없다**: 그 두 노드는 IOKit serial
  family 드라이버가 등록될 때 커널이 쌍으로 만든다. 하나를 만들려면
  DriverKit 드라이버나 kext 를 배포해야 하고, 이는 위의 Windows com0com
  항목과 정확히 같은 이유로 범위 밖이다. 이름만 흉내 낸 심볼릭 링크는
  의미론(`tty.*` 의 DCD 대기 등)을 갖지 못한다.
